...
app.config['SQLALCHEMY_DATABASE_URI'] = environ.get('dbURL') or 'mysql+mysqlconnector://root:root@localhost:3306/recyclingbinDB'
...
class MissionCode(db.Model):
    ...
    def json(self):
        return {"mission_category": self.mission_category, "verification_code": self.verification_code, "datetime_expire": self.datetime_expire, "redeemed": self.redeemed}
...
def get_verification_code(verification_code):
    ...
    verification = db.session.query(MissionCode).filter(MissionCode.verification_code == verification_code).first()
...
def generate_code():
    ...
    new_verification_code = MissionCode(mission_category=query_mission_category, verification_code=random_string, datetime_expire=datetime.now() + timedelta(minutes=15),redeemed=0)
    ...
    # failure case
        ...
        return jsonify(
            {
                "code": 500,
                "message": "An error occurred while inserting verification code." + str(e)
            }
        ), 500
    ...
    return jsonify(
        {
            "code": 201,
            "data": new_verification_code.json()
        }
    ), 201
...
def redeem_code():
    ...
    verification_code = db.session.query(MissionCode).filter(MissionCode.verification_code == query_verification_code).first()
    ...
    # failure case
        ...
        return jsonify(
            {
                "code": 500,
                "message": "An error occurred while updating verification code redemption status." + str(e)
            }
        ), 500
