...
const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    LeaderboardService.getAll().then((users) => {
      setUsers(users);
    });
    window.localStorage.setItem("userID", "162804203307686");
  }, []);
  ...
  useEffect(() => {
    ...
    const index = sortedUsers.findIndex(
      (user) => user.userId === storedMainUser
    );
            ...
            <div className="col-4 m-0 p-0">
              ...
              <img
                ...
                style={{
                  width: "80px",
                  height: "80px",
                  boxShadow: "0 0 6px 2px rgba(0, 255, 0, 0.5)",
                }}
              />
              <br />
              <label>
                <strong>@{sortedUsers[1].name}</strong>
              </label>
              <br />
              <label>
                <strong className="top-three">{sortedUsers[1].points}</strong>
              </label>
            </div>
            <div className="text-center col-4 m-0 p-0">
              ...
              <img
                ...
                style={{
                  width: "100px",
                  height: "100px",
                  boxShadow: "0 0 10px 4px rgba(0, 255, 0, 0.5)",
                }}
              />
              <br />
              <label>
                <strong>@{sortedUsers[0].name}</strong>
              </label>
              <br />
              <label>
                <strong className="top-three">{sortedUsers[0].points}</strong>
              </label>
            </div>
            <div className="col-4 m-0 p-0">
              <div className="mb-2">
              ...
              <br />
              <label>
                <strong>@{sortedUsers[2].name}</strong>
              </label>
              <br />
              <label>
                <strong className="top-three">{sortedUsers[2].points}</strong>
              </label>
            </div>
          </div>
          <br />
          <div
            style={{ height: "475px", overflowY: "scroll" }}
              ...
              {sortedUsers.slice(3).map((user, index) => (
                ...
                >
                  ...
                  <div className="col-10">
                    <div
                      className={`badge badge-pill border border-success p-0 m-0 ${
                        user.userId === mainUser ? "glow" : ""
                      }`}
                    ...
                    >
                      ...
                      />
                      <label className="mx-5" style={{ fontSize: "12px" }}>
                        @{user.name}
                      </label>
                      <label className="points ms-auto pe-3">
